...
def seleccionar_opcion(opcion, datos_juego, incorrect_sound, correct_sound):
    ...
    """
    datos_juego = verificar_respuesta(
        opcion, datos_juego, incorrect_sound, correct_sound
    )
    datos_juego["mensaje_comodin_tiempo"] = pygame.time.get_ticks()
    if (
        datos_juego["pregunta_actual"] is None
        and not datos_juego["doble_chance_activo"]
    ):
        datos_juego["toasty_active"] = True
        datos_juego["toasty_type"] = (
            "correcto" if opcion == datos_juego["respuesta_correcta"] else "incorrecto"
        )
    ...
    if not datos_juego["pregunta_actual"]:
        if not datos_juego["preguntas"]:
            ...
            datos_juego["opciones"] = [
                datos_juego["pregunta_actual"][f"opcion{i+1}"] for i in range(4)
            ]
            random.shuffle(datos_juego["opciones"])
            datos_juego["respuesta_correcta"] = datos_juego["pregunta_actual"][
                "correcta"
            ]
        ...
        tiempo_transcurrido = (
            pygame.time.get_ticks() - datos_juego["tiempo_inicio"]
        ) / 1000
        if (
            tiempo_transcurrido
            > DIFICULTADES[datos_juego["dificultad_seleccionada"]]["tiempo_pregunta"]
        ):
            datos_juego["vidas"] -= 1
            ...
            texto_pregunta = fuente.render(
                datos_juego["pregunta_actual"]["pregunta"], True, NEGRO
            )
            ...
            for i, opcion in enumerate(opciones_a_mostrar):
                dibujar_boton(
                    ...
                    GRIS,
                    AZUL,
                    lambda opt=opcion: datos_juego.update(
                        seleccionar_opcion(
                            opt,
                            datos_juego,
                            datos_juego["incorrect_sound"],
                            datos_juego["correct_sound"],
                        )
                    ),
                )
            ...
            if not datos_juego["bomba_usada"]:
                dibujar_boton(
                    ...
                    fuente,
                    "Bomba",
                    base_x_comodin + idx * (ancho_boton + espacio),
                    base_y_comodin,
                    ancho_boton,
                    50,
                    GRIS,
                    ROJO,
                    lambda: datos_juego.update(
                        usar_bomba(datos_juego),
            ...
            pantalla.blit(
                fuente.render(f"Puntaje: {datos_juego['puntaje']}", True, NEGRO),
                (50, 500),
            )
            pantalla.blit(
                fuente.render(f"Vidas: {datos_juego['vidas']}", True, NEGRO), (50, 550)
            )
            ...
            tiempo_restante = max(
                0,
                int(
                    DIFICULTADES[datos_juego["dificultad_seleccionada"]][
                        "tiempo_pregunta"
                    ]
                    - tiempo_transcurrido
                ),
            ...
            pantalla.blit(
                fuente.render(f"Tiempo: {tiempo_restante}", True, NEGRO), (550, 500)
            )
            if datos_juego["mensaje_comodin"] or datos_juego["doble_chance_activo"]:
                ...
                mensaje_mostrar = (
                    ...
                    if datos_juego["mensaje_comodin"]
                    else "Doble Chance: Selecciona otra opción"
                )
