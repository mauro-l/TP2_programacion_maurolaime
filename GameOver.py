...
def mostrar_game_over(pantalla, eventos, datos_juego, fuente, fuente_game_over):
    ...
    pantalla.blit(
        texto_game_over, (200 + (400 - texto_game_over.get_width()) // 2, 250)
    )
    ...
    pantalla.blit(
        texto_boton,
        (
            300 + (200 - texto_boton.get_width()) // 2,
            350 + (50 - texto_boton.get_height()) // 2,
        ),
    )
    now = pygame.time.get_ticks()
    if (
        rect_boton.collidepoint(mouse)
        and click[0] == 1
        and now - datos_juego["ultimo_click_tiempo"] > 500
    ):
