...
def mostrar_fin_juego(pantalla, eventos, datos_juego, fuente, fuente_game_over):
    ...
    mensaje = (
        "¡Felicidades! Respondiste todas las preguntas"
        if datos_juego["juego_terminado_por_preguntas"]
        else "Fin del juego. Ingresa tu nombre:"
    )
    ...
    for evento in eventos:
        if evento.type == pygame.KEYDOWN:
            if (
                ...
                and datos_juego["entrada_nombre"].strip()
                ...
                len(datos_juego["entrada_nombre"]) < LIMITE_NOMBRE
