...
icono = pygame.image.load("icon.webp")
...
def cargar_recursos():
    ...
    recursos = {}
    try:
        recursos["musica_principal"] = pygame.mixer.Sound("musica.mp3")
        recursos["musica_principal"].set_volume(VOLUMEN_INICIAL)
        recursos["musica_preguntas"] = pygame.mixer.Sound("pista.mp3")
        recursos["musica_preguntas"].set_volume(VOLUMEN_INICIAL)
        recursos["incorrect_sound"] = pygame.mixer.Sound("error.mp3")
        recursos["correct_sound"] = pygame.mixer.Sound("correcto.mp3")
        recursos["game_over_sound"] = pygame.mixer.Sound("game_over.mp3")
        recursos["comience_juego_sound"] = pygame.mixer.Sound("click.mp3")
    ...
    try:
        toasty_gif = Image.open("nerd_emoji.gif")
        toasty_frames = []
    ...
    try:
        recursos["juego_background"] = pygame.transform.scale(
            pygame.image.load("fondo_azul.jpg"), (ANCHO, ALTO)
        )
    except FileNotFoundError:
...
def principal():
    ...
    while datos_juego["ejecutando"]:
        ...
        if (
            datos_juego["estado"] not in [JUEGO, GAME_OVER]
            and now - datos_juego["gif_last_update"] > datos_juego["gif_frame_delay"]
        ):
            datos_juego["gif_frame_index"] = (
                datos_juego["gif_frame_index"] + 1
            ) % datos_juego["gif_frame_count"]
        ...
        if datos_juego["toasty_active"]:
            ...
            if datos_juego["toasty_direction"] == "ida":
                ...
                datos_juego["toasty_x"] -= (ANCHO + 200) / (
                    2.5 * FPS
                )  # ~2.5s para vuelta
            if (
                now - datos_juego["toasty_last_update"]
                > datos_juego["toasty_frame_delay"]
            ):
                datos_juego["toasty_frame_index"] = (
                    datos_juego["toasty_frame_index"] + 1
                ) % datos_juego["toasty_frame_count"]
                datos_juego["toasty_last_update"] = now
            if (
                datos_juego["toasty_direction"] == "vuelta"
                and datos_juego["toasty_x"] <= -100
                or now - datos_juego["toasty_start_time"] > 4500
            ):
        ...
        if (
            ...
            and now - datos_juego["mensaje_comodin_tiempo"] > 2000
        ):
            datos_juego["mensaje_comodin"] = ""
        ...
        if datos_juego["estado"] != estado_anterior:
            if (
                ...
                and datos_juego["musica_preguntas"]
                and datos_juego["musica_sonando"]
            ):
                if (
                    datos_juego["musica_actual"] != datos_juego["musica_preguntas"]
                    or not pygame.mixer.get_busy()
                ):
                    if datos_juego["musica_actual"]:
                        datos_juego["musica_actual"].stop()
                    datos_juego["musica_actual"] = datos_juego["musica_preguntas"]
                    datos_juego["musica_actual"].set_volume(
                        datos_juego["volumen_musica"]
                    )
                    datos_juego["musica_actual"].play(-1)
                ...
                datos_juego["estado"] == INICIO_JUEGO
                and datos_juego["comience_juego_sound"]
            ):
                pygame.mixer.stop()
