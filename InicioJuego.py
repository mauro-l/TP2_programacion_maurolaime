...
def mostrar_inicio_juego(pantalla, eventos, datos_juego, fuente, fuente_game_over):
    ...
    if datos_juego["cuenta_regresiva"] <= 0:
        ...
        texto_animacion = fuente_game_over.render(
            f"El juego comienza en {datos_juego['cuenta_regresiva']}", True, NEGRO
        )
        pantalla.blit(
            texto_animacion, (200 + (400 - texto_animacion.get_width()) // 2, 250)
        )
