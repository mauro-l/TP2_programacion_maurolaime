...
def verificar_respuesta(
    opcion_seleccionada, datos_juego, incorrect_sound, correct_sound
):
    """
    ...
    if opcion_seleccionada == datos_juego["respuesta_correcta"]:
        puntos_ganados = DIFICULTADES[datos_juego["dificultad_seleccionada"]][
            "puntos_aciertos"
        ] * (2 if datos_juego["x2_usado"] else 1)
        ...
        mensaje_comodin = f"¡Correcto! +{puntos_ganados} puntos" + (
            " (X2 activado)" if datos_juego["x2_usado"] else ""
        )
        if correct_sound:
            correct_sound.play()
    else:
        if datos_juego["doble_chance_usado"] and not datos_juego["doble_chance_activo"]:
            ...
            datos_juego["puntaje"] += DIFICULTADES[
                datos_juego["dificultad_seleccionada"]
            ]["puntos_errores"]
            datos_juego["vidas"] -= 1
            datos_juego["correctas_seguidas"] = 0
            datos_juego["pregunta_actual"] = None
...
def usar_bomba(datos_juego):
    ...
    opciones_incorrectas = [
        op for op in datos_juego["opciones"] if op != datos_juego["respuesta_correcta"]
    ]
    if len(opciones_incorrectas) >= 1:
        datos_juego["opciones"] = [
            datos_juego["respuesta_correcta"],
            opciones_incorrectas[0],
        ]
...
def usar_x2(datos_juego):
    ...
    if not datos_juego["x2_usado"]:
        ...
        datos_juego["mensaje_comodin"] = (
            "X2 usado: Siguiente respuesta correcta dará el doble de puntos"
        )
    else:
...
def usar_doble_chance(datos_juego):
    ...
    if not datos_juego["doble_chance_usado"]:
        ...
        datos_juego["mensaje_comodin"] = (
            "Doble Chance usado: Tendrás un segundo intento si fallas"
        )
    else:
...
def guardar_y_reiniciar(datos_juego):
    ...
    """
    if (
        datos_juego["entrada_nombre"].strip()
        and datos_juego["entrada_nombre"].isalpha()
    ):
